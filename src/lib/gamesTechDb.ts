 ...
 * Prices are always overwritten (newer data wins).
 * Metadata (imageUrl, releaseYear) uses COALESCE to preserve existing values.
 */
...
export async function getStats(): Promise<{
  ...
  const [[total], [withPrices], [platforms], [last]] = await Promise.all([
    db.select({ c: sql<number>`count(*)::int` }).from(gamesTech),
    db.select({ c: sql<number>`count(*)::int` }).from(gamesTech)
      .where(sql`loose_price_cents IS NOT NULL OR cib_price_cents IS NOT NULL`),
